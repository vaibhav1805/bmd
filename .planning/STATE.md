...
Phase: Quick Tasks — Post-Release Bugfixes
Status: Quick Task 1 COMPLETE (2026-03-09)
Previous: Phase 26.1 (LLM Discovery Fix) — ALL PLANS COMPLETE
Last activity: 2026-03-09 — Quick Task 1: Fixed discovery algorithm creating edges to non-existent files
  - llm_discovery_integration_test.go: 9 new integration tests
  - TestLLMDiscovery_MonorepoCorpus_FilterPassRate: 68/68 edges pass filter (was 0 before fix)
  - TestLLMDiscovery_Deterministic_3Runs: 3 identical runs verified on monorepo cache
  - TestLLMDiscovery_DiscoverWithClaude_Deterministic_3Runs: API-level determinism with mock Claude
  - TestBuildEdgesFromCacheConfidencePreserved replaces TestBuildEdgesFromCacheConfidenceCap (old test)
  - 1001 tests pass, 2 pre-existing failures unrelated to Phase 26.1
  - Commit: acd768c

All phases complete:
  - Phase 26.1 (LLM Discovery Fix): All 3 plans complete ✓
  - Phase 26 (LLM-Powered Semantic Discovery): All 3 plans complete ✓
  - Phase 25 (Discovery Determinism — Aliases): All 1 plan complete ✓
  - Phase 24 (Discovery Determinism Fix): All 3 plans complete ✓
  - Phase 23 (Discovery Algorithm Optimization): All 4 plans complete ✓
...
Progress: [████████████████████████████████] 26 OF 26 PHASES COMPLETE
...
| Phase 23-discovery-optimization P23-01 | 2 | 2 tasks | 2 files |
| Phase 23-discovery-optimization P23-02 | 3 | 3 tasks | 3 files |
| Phase 23-discovery-optimization P23-03 | 8 | 4 tasks | 2 files |
| Phase 23-discovery-optimization P04 | 12 | 3 tasks | 2 files |
| Phase 24-discovery-determinism P03 | 2 | 2 tasks | 1 files |
| Phase 24-discovery-determinism P02 | 4 | 2 tasks | 1 files |
| Phase 24-discovery-determinism P01 | 5 | 2 tasks | 2 files |
| Phase 25-discovery-determinism-aliases P01 | 11 | 2 tasks | 5 files |
| Phase 26-llm-discovery-fix P01 | 135 | 4 tasks | 8 files |
| Phase 26.1 P03 | 8 | 2 tasks | 2 files |
...
- [Phase 23-discovery-optimization]: convertSemanticToDiscovered uses SignalCoOccurrence as SourceType; convertNERToDiscovered uses SignalMention matching existing NERRelationshipsToRegistry pattern
- [Phase 23-discovery-optimization]: DiscoverRelationships updated to 3 algorithms (co-occurrence+structural+NER); semantic excluded due to BM25 index dependency; DiscoverAndIntegrateRelationships is the full 4-algo entry point
- [Phase 23-02]: --edit flag errors when EDITOR env var is not set (was silently falling back to vi)
- [Phase 23-02]: No-args path prints pending list sorted by confidence + usage hints before saving
- [Phase 23-03]: idx.bm25 accessed directly (same package) — no getter method needed; commands.go and index.go are both package knowledge
- [Phase 23-03]: --min-confidence adjusts all 3 discovery filter tiers proportionally (MinConfidence, -0.05 dual, -0.10 triple)
- [Phase 23-04]: Max confidence strategy for signal aggregation — conservative, prevents noise propagation
- [Phase 23-04]: 3-tier filter: solo>=0.75, dual>=0.70, triple>=0.65 — calibrated for ~70% false positive reduction
- [Phase 23-04]: DiscoverAndIntegrateRelationships uses sync.WaitGroup for parallel algorithm execution
- [Phase 23-04]: Explicit graph edges excluded from filtered output — baseline truth is never re-reviewed
- [Phase 23-04]: --skip-discovery flag for speed (CI, large repos) — explicit-only mode
- [Phase 23-04]: NER+SVO now included in DiscoverRelationships() backward-compat function (3 algorithms)
- [Phase 23-04]: Semantic algorithm requires BM25 index; nil idx gracefully skips semantic step
- [Phase 23-04]: buildTestDocs helper renamed to buildOrchTestDocs in orchestration tests to avoid conflict with hybrid_integration_test.go's buildTestDocs (different signature)
- [Phase 23]: Max confidence strategy for signal aggregation — conservative, prevents noise propagation
- [Phase 23]: 3-tier filter: solo>=0.75, dual>=0.70, triple>=0.65 — calibrated for ~70% false positive reduction
- [Phase 23]: Explicit graph edges excluded from filtered output — baseline truth never re-reviewed
- [Phase 23]: Semantic algorithm requires BM25 index; nil idx gracefully skips semantic step
- [Phase 24-02]: FuzzyComponentMatch uses collect-sort-pick: longest ID first, alpha tiebreak for equal lengths
- [Phase 24-02]: FindComponentsInLine sorts by comp.ID before returning for deterministic edge assignment
- [Phase 24-02]: BuildComponentRegistry uses sorted-key slice iteration for deterministic file reference resolution
- [Phase 24-discovery-determinism]: RC-1 fix: sort extractContextRelationships result by source+target before returning
- [Phase 24-discovery-determinism]: RC-2 fix: database > gateway > service > api > db > config > setup > component priority order for componentTypeKeywords ordered slice
- [Phase 24-discovery-determinism P03]: 10-run property tests prove full determinism: all=597 filtered=198 on every run of 100-doc performance corpus
- [Phase 25-discovery-determinism-aliases]: Phase 25 found 4 non-determinism sources: FuzzyComponentMatch alias step (raw map), findComponentByFile (raw map), cosineSimilarity (IEEE-754 floating-point non-associativity from map iteration), and edgeList/signal ordering in manifest generation
- [Phase 25-discovery-determinism-aliases]: cosineSimilarity sorted-key fix: sort vec1 and vec2 keys before accumulating dot/mag1/mag2 to ensure identical IEEE-754 floating-point results across runs; borderline pairs near 0.35 threshold were flipping
- [Phase 26.1-02]: Remove 0.65 confidence cap in buildEdgesFromCache() — Claude already generates calibrated confidence; capping defeated purpose of LLM reasoning
- [Phase 26.1-02]: SignalLLM solo threshold = 0.50 (vs 0.60 for standard signals) — LLM reasons about document purpose not syntactic patterns
- [Phase 26.1-02]: MinConfidenceDual (0.65) > MinConfidence (0.60) — corroborated edges held to slightly higher bar; solo lowered to accommodate LLM-only discoveries
- [Phase 26.1-01]: claudeClient interface injects LLM dependency so tests use mockClaudeClient (zero API calls in test suite)
- [Phase 26.1-01]: DiscoverWithClaude replaces LLMSemanticRelationships in discovery pipeline; tree building uses TreeNodeFromMarkdown (no pageindex subprocess)
- [Phase 26.1]: Pre-populated LLM cache approach: integration tests simulate Claude responses via buildRealisticLLMCache() to test full filter pipeline without network dependencies
- [Phase 26.1]: TestBuildEdgesFromCacheConfidencePreserved replaces TestBuildEdgesFromCacheConfidenceCap: old test verified removed behavior (cap at 0.65), new test verifies confidence is preserved as-is from Claude
...
|---|-------------|------|--------|-----------|
| 1 | Fix discovery algorithm creating edges to non-existent files | 2026-03-09 | 2a8e55b | [1-fix-discovery-algorithm-creating-edges-t](./quick/1-fix-discovery-algorithm-creating-edges-t/) |

## Session Continuity

Current session: 2026-03-09 — Quick Task 1 COMPLETE
Status: Fixed discovery algorithm creating edges to non-existent files
Work:
  - Quick Task 1: Added file validation to tripleToEdge(), path normalization fix in CmdCrawl()
  - Crawl now correctly traverses full graph (34 nodes from any starting point)
  - All 19 microservices properly discoverable; new regression test prevents recurrence
  - Commit: 2a8e55b

Previous session: 2026-03-08 — Phase 26.1 Complete
Status: Phase 26.1-02: Confidence filtering fixed; LLM-discovered relationships now pass the quality gate
Work:
  - Phase 26.1-02: Removed confidence cap, adjusted thresholds, added SignalLLM special case
  - All changes pre-committed in fdeaf8c (Phase 26.1-01 second commit)
  - 46 filter/orchestration tests passing; build clean

Previous session: 2026-03-07 (resumed) — Phase 24 COMPLETE + Phase 25 Gap-Closure Planned
Status: Phase 24 verified but residual non-determinism identified; Phase 25 research & plan created
Work:
  - **Phase 24 Verification:** All 3 plans executed, determinism proven via property tests on synthetic corpus
  - **Gap Discovery:** Real corpus testing (monorepo-unreferenced-docs) reveals variance: 362 vs 378 filtered edges
  - **Root Cause:** FuzzyComponentMatch alias iteration still uses raw map (not sorted keys)
  - **Phase 25 Created:** 25-RESEARCH.md + 25-01-PLAN.md document gap-closure approach (1 plan, ~15-20 min)
  - **ROADMAP Updated:** Phase 25 added as gap-closure phase
  - **STATE Updated:** Current position moved to Phase 25 (GAP CLOSURE PLANNED)

Previous session: 2026-03-03 (ongoing) — Final Documentation Polish & Cleanup
Status: Post-Phase-20 documentation refinements
Activities:
- ✅ Removed all remaining Phase references from ARCHITECTURE.md (architecture decisions, not implementation)
- ✅ Removed Phase numbers from CONTRIBUTING.md project structure guide for new contributors
- ✅ Updated ARCHITECTURE.md: Fixed deprecated 'bmd registry' command references → 'bmd depends'
- ✅ Updated CONTRIBUTING.md: Removed .planning directory reference, simplified project structure section
- ✅ Updated docs/agents.md: Fixed Python agent example using deprecated 'bmd registry' command
- ✅ Verified: Zero Phase references, Zero .planning references, Zero deprecated 'bmd registry' command examples
- Final commit count: 4 commits in session
  ...
  * 8c29edb (docs: Remove Phase references and deprecated command examples) ← current session
  * 0bc6a18 (docs: Fix deprecated 'bmd registry' command in agent example) ← current session
- Documentation is now 100% user-focused, implementation-agnostic, current, and production-ready
