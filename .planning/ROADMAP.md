...
- [x] **Phase 21: Directory & File Filtering** - Allow users to skip vendor/, dotdirs, and unwanted files during all scanning operations with smart defaults and CLI flags (YOLO execution, completed 2026-03-03)
- [x] **Phase 22: Component-Scoped Graph Construction** - Enable component-level graph construction and BFS-based context aggregation for agent-driven troubleshooting in monorepos (YOLO execution, completed 2026-03-03)
- [x] **Phase 23: Discovery Algorithm Optimization & User Review Pipeline** - Reduce false positives in implicit relationship discovery while maintaining coverage through multi-algorithm orchestration and user-driven validation (completed 2026-03-07)
- [x] **Phase 24: Complete Discovery Determinism Fix** - Eliminate all non-deterministic map iteration in NER and co-occurrence algorithms so identical input always produces identical relationship output (completed 2026-03-07)
- [x] **Phase 25: Discovery Determinism (Alias Collisions)** - Fix remaining non-determinism in FuzzyComponentMatch alias collision handling to achieve full end-to-end determinism on real corpus (completed 2026-03-07)
- [x] **Phase 26: LLM-Powered Semantic Discovery with PageIndex Reasoning** - Enhance relationship discovery with LLM reasoning over PageIndex tree summaries to identify semantic relationships based on document purpose and known components (completed 2026-03-08)
- [ ] **Phase 26.1: Fix LLM Discovery Algorithm (INSERTED)** - Implement PageIndex tree-building and Claude API integration directly in BMD; fix confidence scoring to restore deterministic relationship discovery (urgent fix) [Plan 01-02 complete, Plan 03 pending]
...
### Phase 23: Discovery Algorithm Optimization & User Review Pipeline
**Goal**: Reduce false positives in implicit relationship discovery while maintaining coverage through intelligent algorithm orchestration and user-driven validation
**Depends on**: Phase 22 (component discovery algorithms implemented)
**Requirements**: DISCOVERY-OPT-01, DISCOVERY-FILTER-01, DISCOVERY-REVIEW-01
**Success Criteria** (what must be TRUE):
  1. Four discovery algorithms (co-occurrence, structural, semantic, NER+SVO) run in parallel and merge results with signal aggregation
  2. Quality filtering eliminates ~70% of false positives using confidence + signal agreement thresholds
  3. User review interface (manifest + CLI) accepts/rejects discovered edges, enabling human validation
  4. Final graph merges explicit edges + user-accepted discovered edges with full signal traceability
**Plans**: 4 plans

Plans:
- [ ] 23-01-PLAN.md — Core orchestration: DiscoverAndIntegrateRelationships() with parallel algorithms, filtering logic, signal aggregation (DISCOVERY-OPT-01)
- [ ] 23-02-PLAN.md — User review CLI: bmd relationships-review manifest, YAML serialization, user decision capture (DISCOVERY-REVIEW-01)
- [ ] 23-03-PLAN.md — Pipeline integration: wire discovery into bmd index, add flags (--skip-discovery, --min-confidence) (DISCOVERY-OPT-01)
- [ ] 23-04-PLAN.md — Testing & validation: filtering tests, manifest generation, integration on real monorepos (DISCOVERY-FILTER-01)

### Phase 24: Complete Discovery Determinism Fix
**Goal**: Achieve 100% deterministic relationship discovery — identical input always produces identical output, byte-for-byte, across any number of consecutive runs
**Depends on**: Phase 23 (discovery pipeline fully implemented)
**Requirements**: None (no explicit requirement IDs)
**Success Criteria** (what must be TRUE):
  1. 10 consecutive `bmd index` runs on unchanged code produce identical `all` and `filtered` counts
  2. `.bmd-relationships-discovered.yaml` is byte-for-byte identical across all 10 runs
  3. Property test `TestDiscoveryDeterministic_10Runs` passes in the standard `go test` suite
**Plans**: 3 plans

Plans:
- [ ] 24-01-PLAN.md — Fix extractContextRelationships unsorted drain (RC-1) + componentTypeKeywords map → ordered slice (RC-2)
- [ ] 24-02-PLAN.md — Fix FindComponentsInLine unsorted return (RC-3), FuzzyComponentMatch substring fallback (RC-4), BuildComponentRegistry registry iteration (RC-5)
- [ ] 24-03-PLAN.md — Determinism property tests: 10-run suite for full pipeline, NER, and co-occurrence

### Phase 26: LLM-Powered Semantic Discovery with PageIndex Reasoning
**Goal**: Enhance relationship discovery with LLM reasoning over PageIndex tree summaries to identify semantic relationships based on document purpose and known components
**Depends on**: Phase 25 (full determinism achieved)
**Requirements**: DISCOVERY-LLM-01, DISCOVERY-LLM-02, DISCOVERY-LLM-03
**Success Criteria** (what must be TRUE):
  1. LLMSemanticRelationships function queries PageIndex for each document to identify integration targets
  2. Results are cached by content hash to avoid redundant LLM calls
  3. LLM-discovered edges are properly integrated into the 4-algorithm discovery pipeline
  4. Cache invalidation works correctly when documents change
**Plans**: 3/3 complete

Plans:
- [x] 26-01-PLAN.md — LLM discovery algorithm: pointer bug fix, expanded test coverage (5 new tests), SignalLLM verified
- [x] 26-02-PLAN.md — Pipeline integration: opt-in --llm-discovery flag, PageIndex checks guarded, all tests passing
- [x] 26-03-PLAN.md — End-to-end testing: 3 integration tests, cache round-trip validated, determinism confirmed, planning docs updated
...
| 23. Discovery Algorithm Optimization | 4/4 | Complete    | 2026-03-07 |
| 24. Complete Discovery Determinism Fix | 3/3 | Complete    | 2026-03-07 |
| 25. Discovery Determinism (Alias Collisions) | 1/1 | Complete    | 2026-03-07 |
| 26. LLM-Powered Semantic Discovery | 3/3 | Complete   | 2026-03-08 |

**Current Status: 25 phases complete — Phase 26 planning (3 plans created)**
