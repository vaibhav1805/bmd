...
func CmdCrawl(args []string) error {
	...
	// If dir is not specified but files are given, try to infer dir from the file paths
	// by looking for a .bmd directory up the hierarchy
	workDir := a.Dir
	if workDir == "." && len(a.FromMultiple) > 0 {
		// Try to find the indexed directory by checking if any parent of the first file has .bmd/
		firstFile := a.FromMultiple[0]
		checkPath := firstFile
		for {
			...
			// Check if this directory has a .bmd subdirectory
			if _, err := os.Stat(filepath.Join(checkPath, ".bmd")); err == nil {
				workDir = checkPath
				break
			}
		}
	...
	// Get the absolute path of the indexed directory
	absDirName := filepath.Base(absDir)
	...
	for i, f := range a.FromMultiple {
		...
		if filepath.IsAbs(f) {
			...
			absF = filepath.Join(".", f)
		}
		absF = filepath.Clean(absF)
		absPath, _ := filepath.Abs(absF)

		// Try to extract the relative path from the indexed directory
		// by finding the absolute path of the indexed dir and making the file path relative to it
		relPath, err := filepath.Rel(absDir, absPath)
		if err == nil && !strings.HasPrefix(relPath, "..") {
			// Successfully got a relative path within the indexed directory
			normalized = filepath.ToSlash(relPath)
		} else {
			// Fallback: if not under indexed dir, try simple suffix matching
			// This handles cases where user provides "services/user.md" when indexed dir is "test-data/monorepo"
			// We should look for just the filename components from the right
			searchPath := absDirName + "/"
			if idx := strings.Index(normalized, searchPath); idx != -1 {
				normalized = normalized[idx+len(searchPath):]
			}
		}
...
func loadGraphAndServices(absDir string) (*Database, *Graph, []Component, error) {
	...
	// Component detection moved to graphmd

	// Try to load optional components.yaml config.

	// Component detection moved to graphmd - return empty services
