...
// DiscoverRelationships runs all discovery algorithms (co-occurrence and
// structural patterns) on the given documents and returns a deduplicated,
// aggregated set of discovered edges.
//
...
func DiscoverRelationships(documents []Document, componentNames map[string]string) []*DiscoveredEdge {
	...
	return MergeDiscoveredEdges(coOccEdges, structEdges)
}
...
func MergeDiscoveredEdges(edgeSets ...[]*DiscoveredEdge) []*DiscoveredEdge {
	...
	// Collect results.
	results := make([]*DiscoveredEdge, 0, len(merged))
