...
// componentTypeKeywords maps heading/filename keywords to component types.
var componentTypeKeywords = map[string]string{
	"service":   "service",
	"api":       "api",
	"gateway":   "api",
	"database":  "database",
...
func extractFromDocument(doc *Document, registry map[string]*NERComponent) {
	...
	for _, line := range lines {
		...
		for keyword, cType := range componentTypeKeywords {
...
func inferNERType(filePath, name string) string {
	...
	for keyword, cType := range componentTypeKeywords {
		if strings.Contains(candidates, keyword) {
			return cType
		}
	}
	return "unknown"
